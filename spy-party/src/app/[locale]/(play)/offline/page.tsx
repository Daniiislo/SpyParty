...
import { makeRng, MIN_PLAYERS, randomSeed } from "@/lib/game";
import {
  getTopics,
  pickRandomPair,
  resolvePair,
...
export default function OfflineSetupPage() {
  ...
  const locale = useLocale() as BankLocale;
  ...
  function startGame() {
    ...
    const pair = resolvePair(pickRandomPair(topic, makeRng(`${seed}:pair`)), locale);
    ...
    const setup: OfflineSetup = {
      ...
      wordPair: pair,
      topicName: topicName(topic, locale),
      seed,
    };
  ...
  return (
    <main className="bg-blueprint relative flex min-h-dvh flex-col">
      ...
      <div className="relative mx-auto w-full max-w-lg px-4 py-12 sm:px-6 sm:py-16">
        ...
        >
          <Radar className="size-4" /> {t("startGame")}
        </Button>
